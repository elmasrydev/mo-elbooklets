...
## Payments (Paymob) — backend contract: `mobile-paymob-checkout2.md` (local-only, like the OTP guide)
Students buy a subscription plan through Paymob's **hosted** checkout in a webview. The app never renders a card form and never talks to Paymob's API — it calls `createPaymobCheckout`, opens `checkoutUrl`, then confirms by polling.
- **The whole purchase surface is gated on a backend flag** (`isPaymentAllowed`, per-platform booleans read via `Platform.OS`), owned by `PaymentAccessContext` → `usePaymentAccess()`. It **fails closed**: loading, offline, a server error, or a backend without the field all mean off, and the app then behaves exactly as it did before payments existed (the three paywall notices stay informational). It is **never persisted** — a stored `true` would outlive the server switching to `false` — and is re-read on foreground. `src/lib/paymentAccess.ts` is the single seam: the query is not deployed yet, so `fetchPaymentAccess()` returns `null` there until it is. Debug builds can force it on from InternalSettings, gated by `isDebugMode()` **only** — same rule as the OTP skip.
- **Three paywall entry points, deliberately not merged**: `useSubscriptionGate`'s global modal (study screens) plus local `ConfirmModal`s in `QuizSettingsScreen` and `QuizLessonsScreen` (they bypass an iOS fullScreenModal backdrop bug). Each gains a "Subscribe" action only when the flag is on. `checkSubscription({ skipModal: true })` is also used for **free-plan detection** in both quiz screens — don't change its contract.
- **Confirm the attempt, never the account.** Poll `paymentIntentStatus(reference)` (`usePaymentStatusPoll`, 2s × 15 from `POLL_INTERVAL_MS`/`POLL_MAX_ATTEMPTS`, `network-only`). `me.is_subscribed` is global — already true on a trial or another subject — so it cannot confirm *this* purchase. A `null` status means pending, and **exhausting the window is a soft "still confirming", never a failure** (the webhook may be late; pay-later methods resolve hours later). Only an explicit `failed` ends it badly; show the server's `failureReason` as-is (pre-translated).
- **`checkoutUrl` embeds a client secret** — never log it, never send it to analytics or Crashlytics. Log the reference only.
- **Retry = a fresh mutation and a fresh reference.** Never reopen a used checkout URL. The subscription is created by the webhook, so a `pending` mutation response means nothing yet.
- **Always re-fetch `subscriptionPlans` immediately before checkout** — a stale `planId` or a meanwhile-subscribed subject is rejected. Render plans in server order (cheapest first), read names from `nameAr`/`nameEn` client-side (the cached server-localized `name` goes stale on a language switch), and treat `lessonLimit`/`quizLimitPerDay` of **`null` or `0`** as unlimited (`isUnlimited`). `hasPendingOrder` / `hasFullAccess` replace the buy action entirely. Rules are pure functions in `src/utils/subscriptionPlans.ts` and `src/utils/paymobCheckout.ts`.
- **Store posture**: Egypt has no App Store carve-out for external payment (guideline 3.1.1), and turning the flag on for iOS *after* approval is what guideline 2.3.1 targets — the flag exists so iOS can ship with the surface off.
