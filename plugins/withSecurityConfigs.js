...
`;
      ...
      // 1b. Debug-variant override: Metro serves the JS bundle over plain HTTP on
      // localhost, and android:networkSecurityConfig (set above) overrides the debug
      // manifest's usesCleartextTraffic on Android 7+. Without a debug override the
      // dev build cannot reach Metro and red-screens with "Unable to load script".
      // Debug resources replace main's same-named resource; release keeps the strict
      // config above, so production cleartext rules are unchanged.
      const debugXmlDir = join(projectRoot, 'android', 'app', 'src', 'debug', 'res', 'xml');
      fs.mkdirSync(debugXmlDir, { recursive: true });
      const debugNetworkConfigContent = `<?xml version="1.0" encoding="utf-8"?>
<!-- Generated by plugins/withSecurityConfigs.js — debug builds only, never shipped. -->
<network-security-config>
