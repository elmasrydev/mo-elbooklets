...
import { View, Text, Modal, StyleSheet, Platform, Linking, TouchableOpacity, BackHandler } from 'react-native';
        ...
        <TouchableOpacity style={styles(theme, borderRadius, spacing).container} activeOpacity={1}>
          ...
          <View style={styles(theme, borderRadius, spacing).content}>
            <Text style={styles(theme, borderRadius, spacing).title}>
              {isForceUpdate ? t('forceUpdate.requiredTitle', 'Update Required') : t('forceUpdate.optionalTitle', 'Update Available')}
            </Text>
          ...
          <View style={styles(theme, borderRadius, spacing).buttonContainer}>
            <AppButton 
              title={t('forceUpdate.updateNow', 'Update Now')} 
            ...
            {!isForceUpdate && (
              <TouchableOpacity onPress={handleSkip} style={{ alignItems: 'center', paddingVertical: spacing.sm }}>
                <Text style={{ color: theme.colors.textSecondary, fontSize: fontSizes.md }}>{t('forceUpdate.later', 'Maybe Later')}</Text>
              </TouchableOpacity>
            )}
          </View>
          ...
          {isForceUpdate && (
             <View style={styles(theme, borderRadius, spacing).forceBadge}>
               ...
               <Text style={styles(theme, borderRadius, spacing).forceBadgeText}>
                 {t('forceUpdate.mandatoryUpdate', 'Mandatory Update')}
               </Text>
             </View>
          )}
...
const styles = (theme: any, borderRadius: any = {}, spacing: any = {}) => StyleSheet.create({
  overlay: {
    flex: 1,
