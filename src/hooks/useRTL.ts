 ...
 *
 * The returned object MUST stay referentially stable between renders. It is a
 * dependency of `useCommonStyles` and of the per-screen `useMemo`d stylesheets
 * across the app, so returning a fresh literal each render silently defeated
 * every one of those memos and rebuilt the stylesheets on each render.
 * Direction only changes on a language switch, which reloads the bundle anyway.
 */
export const useRTL = () => {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      paddingStart: (val: number) =>
        isMismatch
          ? getPhysical({ paddingRight: val }, { paddingLeft: val })
          : { paddingStart: val },
